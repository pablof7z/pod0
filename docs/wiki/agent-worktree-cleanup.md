...
The e2e per-podcast publish scenario must assert on the actual signed event's pubkey and sig (via nmp_app_sign_event_for_return) rather than last_published_at, because last_published_at is stamped unconditionally before the publish dispatch and register_podcast_signer_in_kernel has no error return. <!-- [^c1691-380] -->
