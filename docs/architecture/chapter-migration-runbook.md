...
10. Open `Pod0Facade`. Chapter workflow intent and recovery begin from the
    Rust-owned store; Swift contains no legacy chapter-job decoder, importer,
    retirement marker, or restore path.
...
The development-only Swift chapter-job decoder, manifests, backup locations,
retirement mutation, and marker schema were deleted by issue #114. No rollback
may reinsert Swift job rows or reactivate a Swift chapter executor.
