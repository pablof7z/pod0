...
| **Friend / friend-agent messages**       | Yes                       | kind 1, threaded via NIP-10 (`e`/`p` tags) | Friends spec in `ux-12`    | The friend-agent path in `ux-12` §6 explicitly uses *this* identity for self-messages and outgoing. NIP-17 (kind 14/1059) is an explicit non-goal — friend/agent messaging uses public kind:1, matching the `win-the-day-app` reference. |
