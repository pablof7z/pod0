...
key-decisions:
  ...
  - "pod0-tts-host's Cargo.toml keeps a concrete tracing = \"=0.1.44\" literal instead of tracing.workspace = true, matching Plan 01-01's established pattern of keeping concrete version literals in the three previously-self-contained crates (nostr-host/system-hosts/tts-host) so they remain buildable via a bare cd + cargo build outside the parent workspace."
...
**Plan's literal "exactly 1" runtime-construction-grep acceptance criterion conflicts with the plan's own `<behavior>` spec.** Task 1's acceptance criteria states the grep for `Builder::new_current_thread|Builder::new_multi_thread|Runtime::new|#[tokio::main]` across `crates/` should return exactly 1 hit (`host.rs`'s `HostExecutor::new`). The actual, correct result is 4 hits: `host.rs:50` (the intended sole process runtime, now multi-thread), `crates/pod0-portable-media/src/source.rs:115` (`MediaLoader::new`'s owned-Runtime path, which the same task's `<behavior>` block explicitly requires to be preserved unchanged as "an additive constructor, not a breaking change"), `crates/pod0-cli/src/host/playback.rs:427` (a test-local `Runtime` in the updated `player_can_be_constructed` test, which the same task's acceptance-criteria text explicitly instructs adding: "updated to pass a `Handle`, e.g. from a test-local `tokio::runtime::Runtime`"), and `crates/pod0-nostr-host/src/publisher.rs:38` (pre-existing from Plan 01-01, a different crate not in this plan's scope). Resolved in favor of the more detailed `<behavior>` spec and the plan's own test-update instruction — the actual invariant achieved is "no *production* code path in `pod0-cli` builds two runtimes in the same process," which the four hits above satisfy.
