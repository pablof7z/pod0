...
final class AudioSessionCoordinator {
    ...
    /// engine reasons about it directly. `SessionState` is a higher-level
    /// facade Voice mode flips between when a conversation starts or ends —
    /// exactly the two stable shapes called for in the research note
    /// (`docs/spec/research/voice-stt-tts-stack.md` §3):
    ///
    /// - **`.playbackOnly`** → `.playback` + `.spokenAudio`. Episode
    ///   playback; ducks for system spoken audio (Siri / nav).
