use pod0_application::{ChapterPlaybackContext, PlaybackCommand, QueuePlacement};
use pod0_domain::{
    CompletionCause, CompletionStatus, PlaybackSegment, PlaybackSleepMode, QueueEntry,
};
...
fn hash_chapter_action(
    hash: &mut Sha256,
    tag: &[u8],
    context: ChapterPlaybackContext,
    position_milliseconds: u64,
) {
    hash.update(tag);
    hash.update(context.episode_id.into_bytes());
    hash.update(context.artifact_id.into_bytes());
    hash.update(context.selection_revision.value.to_be_bytes());
    hash.update(context.session_id.into_bytes());
    hash.update(context.policy_version.to_be_bytes());
    hash.update(position_milliseconds.to_be_bytes());
}
