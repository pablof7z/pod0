...
impl HostExecutor {
    pub(crate) fn new(config: HostConfig) -> Result<Self, CliError> {
        let live = LiveHosts::new(ClientConfig::default()).map_err(|_| {
            CliError::new("live_hosts", "live host client initialization failed", true)
        })?;
...
pub(super) fn map_adapter_error(error: &AdapterError) -> HostObservation {
    match error {
        ...
        AdapterError::Unavailable(_) | AdapterError::File(_) | AdapterError::Cancelled => failed(
