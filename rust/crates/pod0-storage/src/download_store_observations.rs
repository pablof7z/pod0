...
use crate::{
    DownloadFailureInput, DownloadHostRequestKind, DownloadObservationOutcome, StorageError,
};

#[cfg(test)]
#[path = "download_store_observation_test_support.rs"]
mod test_support;

pub(crate) fn apply_download_host_task(
...
pub(crate) fn apply_download_cancellation(
    transaction: &rusqlite::Transaction<'_>,
    request_id: HostRequestId,
    sequence_number: u64,
    observed_at_ms: i64,
    ...
    if !matches!(
        host.kind,
        DownloadHostRequestKind::Start | DownloadHostRequestKind::Cancel
    ) {
        return Ok(DownloadObservationOutcome::Stale);
    }
...
pub(crate) fn apply_download_artifact_removal(
    transaction: &rusqlite::Transaction<'_>,
    request_id: HostRequestId,
    sequence_number: u64,
    artifact_key: &str,
    ...
    if host.kind != DownloadHostRequestKind::Remove
        || host.artifact_key.as_deref() != Some(artifact_key)
        || current.request_id != Some(request_id)
    {
        return Ok(DownloadObservationOutcome::Stale);
    }
...
pub(crate) fn apply_download_failure(
    transaction: &rusqlite::Transaction<'_>,
