...
impl LibraryStore {
    pub fn claim_model_chapter_submission(
        ...
        crate::transition_commit::commit_model_chapter_submission(self.path(), input)
    }
}

pub(crate) fn apply_model_chapter_submission_claim(
...
impl LibraryStore {
    ...
    pub fn record_model_chapter_provider_accepted(
        ...
        self.write(|transaction| Self::apply_model_chapter_provider_accepted(transaction, input))
    }

    pub(crate) fn apply_model_chapter_provider_accepted(
        transaction: &rusqlite::Transaction<'_>,
        input: ModelChapterProviderAcceptedInput,
    ) -> Result<ModelChapterWorkflowRecord, StorageError> {
        ...
        transaction
            ...
            .map_err(|error| StorageError::sqlite("accept model chapter provider job", error))?;
        if transaction.changes() != 1 {
            return Err(StorageError::ChapterWorkflowConflict);
        }
        read_workflow(transaction, input.episode_id)?.ok_or(StorageError::ChapterWorkflowNotFound)
    }
    ...
    pub fn stage_model_chapter_completion(
        ...
        self.write(|transaction| Self::apply_model_chapter_completion_stage(transaction, input))
    }

    pub(crate) fn apply_model_chapter_completion_stage(
        transaction: &rusqlite::Transaction<'_>,
        input: ModelChapterCompletionInput,
    ) -> Result<ModelChapterCompletionRecord, StorageError> {
...
pub(crate) fn exact_claim_record(
    transaction: &rusqlite::Transaction<'_>,
    input: &ModelChapterSubmissionClaimInput,
) -> Result<ModelChapterWorkflowRecord, StorageError> {
