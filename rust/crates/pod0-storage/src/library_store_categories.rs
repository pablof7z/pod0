use pod0_domain::{
    CategoryId, CategoryOrigin, CommandId, MAX_CATEGORIES, StateRevision, category_slug,
    validate_category, validate_color_hex,
};
use rusqlite::params;

use crate::StorageError;
use crate::category_store_model::encode_origin;
use crate::category_store_read::{active_category_count, category_exists};
...
impl LibraryStore {
    ...
    pub fn create_category(
        ...
        self.write(|transaction| {
            ...
            {
                return Ok((revision, category_id));
            }
            validate_category(name, description, color_hex, origin)
                .map_err(|_| StorageError::InvalidCategory)?;
            if active_category_count(transaction)? >= MAX_CATEGORIES {
                return Err(StorageError::InvalidCategory);
            }
            transaction
                .execute(
                    "INSERT INTO pod0_categories(category_id,category_revision,name,slug,\
                     description,color_hex,origin_code,created_at_ms,updated_at_ms,deleted,\
                     created_command_id) VALUES(?1,1,?2,?3,?4,?5,?6,?7,?7,0,?8)",
                    params![
    ...
    pub fn update_category(
        ...
        self.write(|transaction| {
            ...
            validate_color_hex(edit.color_hex.as_deref())
                .map_err(|_| StorageError::InvalidCategory)?;
            ...
            finish_category_command(transaction, command_id, command_fingerprint, observed_at_ms)
        })
    }
