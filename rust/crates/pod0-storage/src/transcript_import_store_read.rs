...
pub fn read_active_transcript_import(
    target_path: &Path,
) -> Result<Option<TranscriptImportReport>, StorageError> {
    let connection = open_current(target_path)?;
    let import_id: Option<Vec<u8>> = connection
        .query_row(
            "SELECT import_id FROM pod0_transcript_imports \
             WHERE state IN ('staged','verified','corrupt') ORDER BY staged_at_ms LIMIT 1",
            [],
            |row| row.get(0),
        )
        .optional()
    ...
    import_id
        .map(|bytes| {
            let id = CommandId::from_bytes(bytes.try_into().map_err(|_| {
                StorageError::CorruptSchema {
                    detail: "transcript import identity is malformed",
                }
            })?);
            read_import_report(&connection, id, true)?.ok_or(StorageError::TranscriptImportNotFound)
        })
        .transpose()
}
...
pub fn read_transcript_import_entries(
    ...
    let mut statement = connection
        .prepare(
            "SELECT e.episode_id,e.is_selected,e.selected_row_digest,e.selected_file_digest,\
             e.artifact_id,e.transcript_version_id,d.content_digest \
             FROM pod0_transcript_import_entries e \
             JOIN pod0_transcript_documents d ON d.transcript_version_id=e.transcript_version_id \
             WHERE e.import_id=?1 ORDER BY e.episode_id,e.legacy_row_id LIMIT ?2 OFFSET ?3",
        )
        .map_err(|error| {
    ...
    let items = rows
        .map(|row| {
            ...
            Ok(TranscriptImportEntrySummary {
                ...
                artifact_id: artifact_id(&row.4)?,
                transcript_version_id: version_id(&row.5)?,
                transcript_content_digest: digest(&row.6)?,
            })
        })
...
pub(crate) fn read_import_report(
    ...
    let row = connection
        .query_row(
            "SELECT source_kind,source_generation,source_database_digest,\
             ...
             artifact_count,selected_count,target_revision,state,diagnostic_code \
             FROM pod0_transcript_imports \
             WHERE import_id=?1",
    ...
    let selected_count =
        u32::try_from(row.7).map_err(|_| corrupt("selected transcript count is malformed"))?;
    ...
    if u32::try_from(aggregates.0).ok() != Some(artifact_count) {
        return Err(corrupt("transcript import entry count differs"));
    }
    let stored_selected: i64 = connection
        .query_row(
            "SELECT COUNT(*) FROM pod0_transcript_import_entries \
             WHERE import_id=?1 AND is_selected=1",
            [import_id.into_bytes().as_slice()],
            |row| row.get(0),
        )
        .map_err(|error| StorageError::sqlite("read selected transcript import count", error))?;
    if u32::try_from(stored_selected).ok() != Some(selected_count) {
        return Err(corrupt("selected transcript import count differs"));
    }
    ...
    Ok(Some(TranscriptImportReport {
        ...
        target_revision: StateRevision::new(stored_u64(row.8, "transcript target revision")?),
        backup: TranscriptBackupEvidence {
            ...
            reused_artifacts: if reused_existing { artifact_count } else { 0 },
        },
        state: TranscriptImportState::from_code(&row.9)
            .ok_or_else(|| corrupt("transcript import state is malformed"))?,
        diagnostic_code: row.10,
        reused_existing,
    }))
}
...
pub(crate) fn read_import_entries(
    ...
    let mut statement = connection
        .prepare(
            "SELECT episode_id,is_selected,legacy_row_id,selected_row_digest,\
             selected_file_digest,backup_file_digest,backup_file_byte_count,artifact_id,\
             transcript_version_id FROM pod0_transcript_import_entries WHERE import_id=?1 \
             ORDER BY episode_id,legacy_row_id",
        )
    ...
    rows.map(|row| {
        ...
        Ok(StoredTranscriptImportEntry {
            episode_id: episode_id(&row.0)?,
            is_selected: row.1,
            legacy_row_id: stored_u64(row.2, "transcript legacy row identity")?,
            selected_row_digest: digest(&row.3)?,
            selected_file_digest: digest(&row.4)?,
            backup_file_digest: digest(&row.5)?,
            backup_file_byte_count: stored_u64(row.6, "transcript artifact backup bytes")?,
            artifact_id: artifact_id(&row.7)?,
            transcript_version_id: version_id(&row.8)?,
        })
    })
    .collect()
}
