...
fn playback_slice_commits_selection_queue_resume_and_natural_advance_atomically() {
    ...
    let activity = crate::ActivityStore::open(&fixture.target)
        .unwrap()
        .page_for_episode(second, None, 20)
        .unwrap();
    ...
    assert_eq!(
        activity.items.iter().filter(|item| matches!(
            item.draft.fact,
            pod0_application::ActivityFact::DomainTransition {
                kind: pod0_application::DomainTransitionKind::Playback(
                    PlaybackTransition::QueueChanged
...
fn session_sleep_timer_is_cleared_once_without_erasing_other_playback_state() {
    ...
    let cleared = store.clear_session_sleep_timer(1_800_000_000_003).unwrap();
    let after = store.snapshot().unwrap();
    assert_eq!(after.playback.sleep_mode, PlaybackSleepMode::Off);
    assert_eq!(after.playback.active_episode_id, Some(episode_id));
    assert_eq!(
        store.clear_session_sleep_timer(1_800_000_000_004).unwrap(),
        cleared
    );
}
