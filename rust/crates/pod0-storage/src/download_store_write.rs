...
impl LibraryStore {
    pub fn ensure_download_workflow(
        ...
        crate::transition_commit::commit_download_admission(self.path(), input)
    }

    pub fn ensure_download_workflow_from_internal_command(
        &self,
        command: crate::PendingInternalCommand,
        input: DownloadEnsureInput,
    ) -> Result<DownloadEnsureOutcome, StorageError> {
        crate::transition_commit::commit_download_internal_admission(self.path(), command, input)
    }

    pub fn record_download_internal_disposition(
        &self,
        command: crate::PendingInternalCommand,
        ...
        crate::transition_commit::commit_download_internal_disposition(
            self.path(),
            command,
            episode_id,
            disposition,
...
pub(crate) fn apply_download_ensure(
    transaction: &Transaction<'_>,
    input: DownloadEnsureInput,
) -> Result<DownloadEnsureOutcome, StorageError> {
    if download_command_was_applied(transaction, input.command_id, &input.command_fingerprint)?
        .is_some()
    {
...
impl LibraryStore {
    ...
    pub fn record_download_noop_command(
        ...
        subject: pod0_application::ActivitySubject,
        episode_id: Option<EpisodeId>,
        origin: pod0_application::DownloadIntentOrigin,
        internal_commands: Vec<pod0_application::DurableInternalCommandRequest>,
        observed_at_ms: i64,
    ) -> Result<StateRevision, StorageError> {
        crate::transition_commit::commit_download_noop(
            self.path(),
            command_id,
            command_fingerprint,
            subject,
