-- Allow the dedicated exact native-cancellation effect kind (16) while
-- preserving every existing intent, attempt, fence, and observation.
CREATE TABLE pod0_effect_intents_v42(
    ...
    effect_kind_code INTEGER NOT NULL CHECK(effect_kind_code BETWEEN 1 AND 16),
...
CREATE TABLE pod0_effect_attempts_v42(
    attempt_id BLOB PRIMARY KEY NOT NULL CHECK(length(attempt_id)=16),
    intent_id BLOB NOT NULL REFERENCES pod0_effect_intents_v42(intent_id),
...
INSERT INTO pod0_effect_intents_v42 SELECT * FROM pod0_effect_intents;
INSERT INTO pod0_effect_attempts_v42 SELECT * FROM pod0_effect_attempts;
DROP TABLE pod0_effect_attempts;
DROP TABLE pod0_effect_intents;
ALTER TABLE pod0_effect_intents_v42 RENAME TO pod0_effect_intents;
ALTER TABLE pod0_effect_attempts_v42 RENAME TO pod0_effect_attempts;
